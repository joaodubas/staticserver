...
(function (wdw, doc, undefined) {
  ...
  Roller.prototype.setControls = function setControls() {
    ...
    function move(e) {
      if (e.target.nodeName.toLowerCase() !== 'button' || self.walker.walking) {
        return;
      }
  ...
  Roller.prototype.setAnimation = function setAnimation() {
    ...
    if (Modernizr.csstransitions) {
      this.roller.addEventListener('webkitTransitionEnd', endAnimation);
      this.roller.addEventListener('transitionend', endAnimation);
    } else {
...
(function (wdw, doc, undefined) {
  ...
  carousel.carousels.forEach(function (roller) {
    $('.widget-details', roller.container).innerHTML = 'Columns ' + $('.widget-item', roller.container).length;
  });
